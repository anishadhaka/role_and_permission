...
@section('content')
    ...
    <form action="{{ route('domain.update', $domain->id) }}" method="POST">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" name="authentication" class="form-control" id="authentication" 
                           value="{{ old('authentication', $domain->authentication) }}" required>
                </div>

                <div class="form-group">
                    <label for="user_name">User Name</label>
                    <input type="text" name="user_name" class="form-control" id="user_name" 
                           value="{{ old('user_name', $domain->user_name) }}" required>
                </div>

                <div class="form-group">
                    <label for="to_mail_id">To Mail ID</label>
                    <input type="text" name="to_mail_id" class="form-control" id="to_mail_id" 
                           value="{{ old('to_mail_id', $domain->to_mail_id) }}" required>
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="mail_header" class="form-control rich-text-editor" id="mail_header">{{ old('mail_header', $domain->mail_header) }}</textarea>
        </div>

        <div class="form-group">
            <label for="mail_footer">Mail Footer</label>
            <textarea name="mail_footer" class="form-control rich-text-editor" id="mail_footer">{{ old('mail_footer', $domain->mail_footer) }}</textarea>
        </div>

        <!-- Action Buttons -->
        <button type="submit" class="btn btn-primary mt-3">Update Domain</button>
