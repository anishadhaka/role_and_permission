...
<table class="table table-bordered">
    ...
    <tr>
        ...
        <td>
            <select id="select" class="form-control status-dropdown" data-id="{{ $news->id }}" style="width: auto;">
                ...
                <option id="option" value="{{ $item->id }}"
                    data-deginationid="{{ $news->approvednewsstatus->designation_id }}" data-userid="{{ $item->id }} "
                    <?php  if ($news->approvednewsstatus->designation_id == $item->id ){ echo 'selected'; }else{ echo 'disabled'; } ?>>
        ...
        <th style="display: flex; border: none;">
            <form action="{{ route('approve', $news->id) }}" method="POST" style="display:inline"
